...
48. **🚨 Le prompt libre AVEC sélection n'écrivait pas dans le document.** Suite directe du n°47 : la case « Ajouter à la suite » semblait toujours inopérante, mais pour une autre raison. Le chemin déterministe n'avait été branché que sur le cas « aucune sélection ⇒ document entier ». Avec une sélection, la demande repartait en **mode agentique**, dont le sink par défaut est la **palette** : le texte s'affichait dans la fenêtre, le document restait inchangé, et la case n'avait rien à piloter.
    **Diagnostic** : télémétrie `preset=free` et **absence** de la trace « Document réécrit » — le run a bien tourné, mais par le mauvais chemin. Chercher quel chemin a été pris avant de chercher pourquoi il a échoué.
    **Symétrie à respecter** : les deux portées — sélection et document entier — doivent avoir chacune leur chemin déterministe, et **partager le même traducteur de choix** (`presets.text_sink`). Un test d'architecture le vérifie par introspection du code source : si un jour la réécriture de sélection cesse de passer par le helper, la case redevient silencieusement inopérante sur ce chemin, et le test échoue.
    **Leçon transversale de la série n°44/45/47/48** : une même plainte utilisateur — « ça ne fait rien » — a recouvert quatre causes différentes (verbe non reconnu, modèle qui n'enchaîne pas, réglage débranché, chemin sans écriture). Ne jamais supposer qu'un symptôme déjà rencontré a la même cause que la fois précédente : **re-mesurer à chaque fois quel chemin a été emprunté.**

49. **🚨 Un validateur peut être tolérant sur un axe et intransigeant sur l'axe voisin, sans que personne ne le remarque.** Journal des actions : `✗ Lecture du document (11 ms) — paramètre 'max_chars' : maximum 20000`. Le modèle avait demandé une lecture plus large que le plafond ; le validateur **refusait l'appel entier**. Le modèle perdait un tour sur un paramètre de **confort** — il voulait « le plus possible », pas une taille précise — et, avec un petit modèle, un tour perdu se solde souvent par un abandon pur et simple.
    **L'asymétrie**, dans le même bloc de code, à douze lignes d'écart : une chaîne trop longue était **tronquée** en silence (`maxLength`), un nombre hors bornes était **rejeté** (`minimum`/`maximum`). Deux contraintes de même nature — un plafond de taille — traitées de façon opposée. Personne ne l'a vu parce que chaque branche, prise isolément, se défend.
    **Remède** : ramener dans les bornes, comme on tronque déjà. En arrondissant **vers l'intérieur** (`ceil` pour un plancher, `floor` pour un plafond) : un simple `int(borne)` retomberait hors bornes dès que la borne est fractionnaire — `int(0.5) == 0`, soit toujours sous un minimum de 0,5. Un test le verrouille.
...
51. **Un journal qui ne va qu'à l'écran ne sert à rien pour diagnostiquer.** Constaté en recette : l'utilisateur signale un défaut, `~/log.txt` ne porte que `run: début` et `run: terminé`. Impossible de dire quel chemin a été emprunté ni si le document a été modifié — les lignes utiles (`✓ Lecture du document — 45 paragraphe(s)`, `⚙ Réécriture des paragraphes 2 à 46`) n'existaient que dans l'onglet Actions, donc uniquement chez l'utilisateur, et disparaissaient à la fermeture. C'est précisément l'information qui manquait pour appliquer le n°48 (« mesurer quel chemin a été pris »).
    **Trois destinations, trois publics** — et c'est la distinction qui compte : l'**onglet Actions** pour l'utilisateur, en français ; **`~/log.txt`** pour le diagnostic, systématique ; la **télémétrie** pour l'exploitation, *sur étape nommée seulement*.
    **Le piège dans le remède** : le journal CITE le document (« ↳ Titre conservé : « Rapport annuel 2026 » »). Le fichier reste sur le poste, la télémétrie **part sur le réseau**. Verser mécaniquement les lignes du journal dans les traces exfiltrerait du contenu documentaire. La parade n'est pas la vigilance à chaque appel — elle s'érode — mais un **filtre qui ne fait pas confiance à l'appelant** : seuls passent les nombres, les booléens et les étiquettes d'un vocabulaire fermé ; toute chaîne libre est écartée par construction (`core/telemetry_steps.py`, verrouillé par test avec un vrai titre de document en entrée).
    **Corollaire mesuré au passage** : l'encodeur OTLP appliquait `str()` à tout attribut. « 45 paragraphes » arrivait dans Tempo en `stringValue: "45"` — lisible, mais ni agrégeable ni seuillable. Une trace fonctionnelle dont les mesures sont des chaînes n'est pas une mesure. Typage rétabli (`intValue`/`boolValue`/`doubleValue`), avec `bool` testé **avant** `int` puisque `True` est un entier en Python.
